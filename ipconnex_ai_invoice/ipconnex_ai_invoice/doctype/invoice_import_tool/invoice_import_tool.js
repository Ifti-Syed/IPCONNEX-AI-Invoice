  ...
  refresh: function (frm) {
    ...
    $("button[data-fieldname='extract_data']")
      ...
      .on("click", (e) => {
        ...
        frappe.call({
          method:
            "ipconnex_ai_invoice.ipconnex_ai_invoice.extract.extractPDFData",
          args: {
            doc_name:
              frm.doc.invoice_type == "Purchase" ? "Supplier" : "Customer",
          ...
          callback: function (response) {
            ...
            if (res_json["status"]) {
              ...
              try {
                frm.set_value({
                  extracted_amount: invoice_data["total_amount"],
                });
              } catch (e) {
                console.log(e);
              }
              ...
              try {
                ...
                for (let i in items) {
                  if (items[i].amount != 0) {
                    ...
                    if (
                      !isNaN(rate_float) &&
                      !isNaN(amount_float) &&
                      !isNaN(duration_float)
                    ) {
                      if (
                        Math.abs(
                          (rate_float * amount_float) / duration_float - 1
                        ) <
                          Math.abs(
                            (rate_float * duration_float) / amount_float - 1
                          ) &&
                        Math.abs(
                          (rate_float * amount_float) / duration_float - 1
                        ) < 0.1
                ...
                frm.set_value({
                  invoice_total_amount: amount / 100,
                  difference:
                    Math.abs(
                      amount - Math.round(invoice_data["total_amount"] * 100)
                    ) / 100,
                });
              } catch (e) {
                console.log(e);
  ...
  item_qty: function (frm, cdt, cdn) {
    setTimeout(function () {
      ...
      frm.set_value({
        invoice_total_amount: amount / 100,
        difference:
          Math.abs(amount - Math.round(frm.doc.extracted_amount * 100)) / 100,
      });
    }, 300);
  },
