...
def extractPDFData(doc_name,pdf_path,account_name):
    ...
    try:
        question = "Please give me all details of this invoice in an JSON format. I want the JSON to contain this info with this keys: {'company': company , 'invoice_date': 'yyyy-mm-dd', 'invoice_items': [{'item_description': item_description ,'rate': rate ,'duration':duration ,'amount': charges}]} . items must be an array of objects, adapt for charges written with space each 3 numbers and make sure the result could be parsed as json . Here's a list of all customers, choose the one that fits best: " + ', '.join(companies) +  ". and here's a text extracted from an invoice pdf document: " + pdf_text
